...
class Board:
	...
	def is_valid(self, d):
                #player + direction not in walls, then
		if self.player + d.sp not in self.walls:
                        #player + direction is a box
			if self.player + d.sp in self.boxes:
                                #player + direction*2 not a wall or a box, then true
				if self.player + d.sp.double() not in self.boxes.union(self.walls):
					return True
			else:   #player + direction not a wall and not a box
				return True
                #it is a wall
		return False
